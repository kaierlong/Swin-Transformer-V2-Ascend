...
from src.trainers import TrainClipGrad, TrainEMA
...
def get_train_one_step(args, net_with_loss, optimizer):
    ...
    if args.enable_ema:
        net_with_loss = TrainEMA(
            net_with_loss, optimizer, scale_sense=scale_sense,
            enable_ema=args.enable_ema, ema_decay=args.ema_decay,
            clip_global_norm_value=args.clip_global_norm_value)
    elif args.use_global_norm:
        net_with_loss = TrainClipGrad(
            net_with_loss, optimizer, scale_sense=scale_sense,
            clip_global_norm_value=args.clip_global_norm_value, use_global_norm=True)
    else:
        net_with_loss = nn.TrainOneStepWithLossScaleCell(net_with_loss, optimizer, scale_sense=scale_sense)
